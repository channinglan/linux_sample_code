...
gint swap_rate_ava(void){
	...
	swap_info[0] = strtok (buf, delim);
		...
		swap_info[i]=strtok(NULL,delim);
       	...
       	temp1=strstr(swap_info[20],":");
       	swap_info[20]=temp1 +2;

        temp2=strstr(swap_info[21],":");
       	swap_info[21]=temp2 +2;
        SwapTotal=atoi(swap_info[20]);
        SwapFree=atoi(swap_info[21]);
        swap_rate=100*(1-((float)SwapFree/SwapTotal));//计算交换区使用率
      
      	sprintf(show,"%.2f",swap_rate);
        strcat(text_swap,show);
        strcat(text_swap,"%");
	gtk_label_set_text(GTK_LABEL(swap_rate_label),text_swap);//将结果写入标签
	gtk_progress_bar_set_fraction (GTK_PROGRESS_BAR (swap_bar),swap_rate/100.0);//将结果在进度条上显示
