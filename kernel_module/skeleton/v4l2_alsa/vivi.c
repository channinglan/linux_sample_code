...
#define vivi_v4l2_printk(fmt, arg...) \
	printk(fmt, ## arg)
...
buffer_setup(struct videobuf_queue *vq, unsigned int *count, unsigned int *size)
{
        ...
        *size = dev->width * dev->height * 2;
        ...
        dprintk(dev, 2, "%s, count=%d, size=%d\n", __func__,
                *count, *size);
...
static void free_buffer(struct videobuf_queue *vq, struct vivi_buffer *buf)
{
        ...
        dprintk(dev, 2, "%s, state: %i\n", __func__, buf->vb.state);

        videobuf_vmalloc_free(&buf->vb);
        dprintk(dev, 2, "free_buffer: freed\n");
                                        ...
                                        struct v4l2_capability *cap)
{
	...
	//fix Bad caps for driver vivi, issue (kernel 4.4.0)                         
	cap->device_caps = V4L2_CAP_READWRITE | V4L2_CAP_STREAMING | V4L2_CAP_AUDIO | V4L2_CAP_VIDEO_CAPTURE;;
	cap->capabilities = cap->device_caps | V4L2_CAP_DEVICE_CAPS;	
	
	dprintk(dev, 3,"cap->device_caps=0x%x", cap->device_caps);
	dprintk(dev, 3,"cap->version=0x%x", cap->version);
	dprintk(dev, 3,"cap->capabilities=0x%x", cap->capabilities); 
                                        ...
                                        struct v4l2_format *f)
{
	...
	dprintk(dev, 2,"f->fmt.pix w=%d h=%d field=%d pixelformat=0x%x bytesperline=%d sizeimage=0x%x",	
								f->fmt.pix.width,
								f->fmt.pix.height, 
								f->fmt.pix.field,
                                ...
                                struct v4l2_control *ctrl)
{
 ...
 static int __init vivi_create_instance(int inst,struct pci_dev *pdev)
 {
         struct vivi_dev *dev;
         struct video_device *vfd;
         int ret;
...
//         vfd->debug = debug;
  ...
  */
int  vivi_init(struct pci_dev *pdev)
 {
	...
	vivi_v4l2_printk("%s\n", __func__);
         ...
         for (i = 0; i < n_devs; i++) {
                 ret = vivi_create_instance(i,pdev);
                 if (ret) {
