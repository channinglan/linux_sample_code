 ...
 */
static int devno = 0;
int  alsa_card_minivosc_init(struct pci_dev *pdev)
{
	...
	int dev = devno; // from aloop-kernel.c
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 16, 0)
	snd_card_set_dev(card, &pdev->dev); // present in dummy, not in aloop though
#endif
#if 0
	ret = snd_device_new(card, SNDRV_DEV_LOWLEVEL, mydev, &dev_ops);
	if (ret < 0) {
		dbg("snd_device_new  err %d", ret);
		goto __nodev;
	}
#endif
	...
	*/
	...
	if (ret < 0) {
		dbg("snd_pcm_lib_preallocate_pages_for_all  err %d", ret);
		goto __nodev;
	}
	// * will use the snd_card_register form from aloop-kernel.c/dummy.c here..
	ret = snd_card_register(card);
	if (ret < 0) {
		dbg("snd_card_register  err %d", ret);
		goto __nodev;
	}
	dbg("dev ok!!");
	return ret;
...
//static void __exit alsa_card_minivosc_exit(void)
void  alsa_card_minivosc_exit(void)
{
	dbg("%s", __func__);
